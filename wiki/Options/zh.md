...
在主菜单中，点击 `Options` 或按下 `O` (或者在大部分情况按下 `Ctrl` + `O`) 以显示屏幕边侧的选项菜单并改变osu!的设置。如果选项菜单打开了，可以输入字符以搜索特定项设置。如果找不到对应设置，搜索栏的文字会抖动。

当一个设置不同于默认值时，那一行的左侧会有黄色的光。点击该处会让设置重置至默认值。

## 通用

![General icon](img/general.jpg "General icon")

这一部分的设置包括你的账号、语言设置和你的游戏客户端想收到何种更新。

### 登录

如果你尚未登录游戏客户端，游戏会自动打开选项菜单，提示你登录。你可以通过按下 `Esc` clicking on the back button, or navigating to the song selection screen来忽略此提示. By not signing in, the game client will mark you as a "Guest" and you will not be able to fetch online scores, submit scores, or play [Multi](/wiki/Multi).
